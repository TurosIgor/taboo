...
export default function TeamForm ({ team, dispatch, teams }) {
    ...
    return (
        <div className="Team">
            {renamingTeam ? <><input className="TeamNameInput" defaultValue={team.name} onChange={e => dispatch({type: "RENAME_TEAM", teamId: team.id, name: e.target.value})} onBlur={e => setRenamingTeam(false)} /><button className="SaveButton" onClick={e => setRenamingTeam(false)}>Save</button></> : <><h3 className="TeamName">{team.name}</h3>
            <button type="button" className="RenameTeamButton" onClick={e => setRenamingTeam(true)}>Rename</button></>}
            <div className="Players">
                {team.players.map(player => (
                <div key={player.id} className="Player">
                    {renamingPlayer && renamingPlayer === player.id ? <input className="PlayerNameInput" defaultValue={player.name} onChange={e => dispatch({type: "RENAME_PLAYER", teamId: team.id, playerId: player.id, name: e.target.value})} onBlur={e => setRenamingPlayer(undefined)} /> : <h4 className="PlayerName">{player.name}</h4>}
            ...
            <button disabled={players.length > 9} type="button" className="AddPlayer" onClick={e => dispatch({type: "ADD_PLAYER", teamId: team.id, player:{ id: `player_${getPlayerId()}`, name: `Player ${getPlayerId()}`}})} >Add Player</button>
        </div>
    )
}